...
public void GenerateEGFExport()
        {   
            string fn = tourDirectory + TournamentName.Trim().Replace(" ","") + ".h9";
            ...
            using (StreamWriter sw = new StreamWriter(fn))
            {
                ...
                sw.WriteLine("; Generated by Miasma");
                sw.WriteLine("; ");
                foreach (Player ap in AllPlayers)
                {
...
public void ConvertStandingsToHTML(int rnd)
		{
            string fName = tourDirectory + "Round" + rnd + "Standings.txt";
...
public string CorrelationRatingMMS()
{
  ...
  for (int i = 0; i < RAT.Length; i++) array_xy[i] = RAT[i] * MMS[i];
  for (int i = 0; i < RAT.Length; i++) array_xp2[i] = Math.Pow(RAT[i], 2.0);
  for (int i = 0; i < RAT.Length; i++) array_yp2[i] = Math.Pow(MMS[i], 2.0);
  double sum_x = 0;
  double sum_y = 0;
  foreach (double n in RAT) sum_x += n;
  foreach (double n in MMS) sum_y += n;
  double sum_xy = 0;
